...
def handle_file():
    with open(MEDIA_ROOT + '/bibliotecas.json') as file:
        ...
        for raw_centro in data:
            provincia = get_provincia(raw_centro)
            localidad = get_localidad(raw_centro, provincia)
...
def get_localidad(raw_centro, provincia):
    codigos = getCodigosMunicipioProvincia()
...
def add_mappings(codigos, mappings):
        for mapping in mappings:
            codigos[mapping[0]] = codigos[mapping[1]]

def getCodigosMunicipioProvincia():
